...
combine_Gene_set_PRS <- function(Extra_analyses, Gene_regions, Training_name, Validation_name, All_PRS){
  if (Extra_analyses == TRUE){
    ...
    if (Gene_regions == "both" | Gene_regions == "extended"){
     ...
     cols <- names(Gene_regions_extended)[3:ncol(Gene_regions_extended)]
     setnames(Gene_regions_extended, old = c(3:ncol(Gene_regions_extended)), new = paste("extended_geneset", cols, sep = "_"))
    ...
    if (Gene_regions == "both" | Gene_regions == "normal"){
      ...
      cols <- names(Gene_regions_normal)[3:ncol(Gene_regions_normal)]
      setnames(Gene_regions_normal, old = c(3:ncol(Gene_regions_normal)), new = paste("normal_geneset", cols, sep = "_"))

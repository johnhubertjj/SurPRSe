...
chmod  u+rwx ${path_to_gene_scripts}Genes_MAGMA_annotation_script.sh 
fi
...
parallel ${path_to_gene_scripts}Genes_MAGMA_annotation_script.sh ::: ${Chromosomes_to_analyse[@]} ::: ${Directory_to_work_from} ::: ${path_to_scripts} ::: ${path_to_gene_scripts} ::: ${system}
fi
...
if [ ${Using_raven} == "FALSE" ]; then
 chmod -R g+rx ${path_to_scripts} 
else
 chmod -R u+rx ${path_to_scripts}
fi
...
#I got here in the script, need to assign LDscore to the servers from home directory ldsc=(/home/c1020109/ldsc/ldsc.py) and ensure that all the analysis is complete
Gene_regions=normal

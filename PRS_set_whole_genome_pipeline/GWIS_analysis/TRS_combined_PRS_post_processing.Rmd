---
title: "for_antonio_combined_PRS"
...
```{r}
Name_of_analysis <- "COGS_metabolism_combined_PRS"
```
...
Location_of_analysis <- ("~/Documents/COGS_pathways_results/forantonio_metabolism_combined_PRS/")

# Location of whole genome GWAS1 files
Location_of_whole_genome_CLOZ_GWAS <- paste0("~/Documents/COGS_pathways_results/forantonio_metabolism_combined_PRS/forantonio_metabolism/CLOZ_whole_genome/PRS_scoring/CLOZ_re.EUR.mlma_metabolism_COGS_whole_genome_significance_threshold_at_",significance_threshold,".profile")

# Location of whole genome GWAS2 files
Location_of_whole_genome_RATIO_GWAS <- paste0("~/Documents/COGS_pathways_results/forantonio_metabolism_combined_PRS/forantonio_metabolism/RATIO_whole_genome/PRS_scoring/RATIO_re.EUR.mlma_COGS_whole_genome_significance_threshold_at_",significance_threshold,".profile")

# Location of whole genome GWAS3 files
Location_of_whole_genome_NORCLOZ_GWAS <- paste0("~/Documents/COGS_pathways_results/forantonio_metabolism_combined_PRS/forantonio_metabolism/NORCLOZ_whole_genome/NORCLOZ.EUR.mlma_metabolism_COGS_output/PRS_scoring/NORCLOZ.EUR.mlma_metabolism_COGS_whole_genome_significance_threshold_at_",significance_threshold,".profile")
...
# CLOZ data
my_data <- lapply(Location_of_whole_genome_CLOZ_GWAS, fread, header=TRUE) 
names(my_data) <- str_replace(Location_of_whole_genome_CLOZ_GWAS, pattern = ".profile", replacement = "")
...
all_prs_CLOZ <- join_all(my_data, by=c("FID", "IID"), type='left')

cols <- names(all_prs_CLOZ)[3:ncol(all_prs_CLOZ)]
setnames(all_prs_CLOZ, old = c(3:ncol(all_prs_CLOZ)), new = paste("CLOZ_WHOLE.genome", cols, sep = "_"))

# Ratio data
my_data <- lapply(Location_of_whole_genome_RATIO_GWAS, fread, header=TRUE) 
names(my_data) <- str_replace(Location_of_whole_genome_RATIO_GWAS, pattern = ".profile", replacement = "")
...
all_prs_RATIO <- join_all(my_data, by=c("FID", "IID"), type='left')

cols <- names(all_prs_RATIO)[3:ncol(all_prs_RATIO)]
setnames(all_prs_RATIO, old = c(3:ncol(all_prs_RATIO)), new = paste("RATIO_WHOLE.genome", cols, sep = "_"))

# Norcloz data
my_data <- lapply(Location_of_whole_genome_NORCLOZ_GWAS, fread, header=TRUE) 
names(my_data) <- str_replace(Location_of_whole_genome_NORCLOZ_GWAS, pattern = ".profile", replacement = "")
...
all_prs_NORCLOZ <- join_all(my_data, by=c("FID", "IID"), type='left')

cols <- names(all_prs_NORCLOZ)[3:ncol(all_prs_NORCLOZ)]
setnames(all_prs_NORCLOZ, old = c(3:ncol(all_prs_NORCLOZ)), new = paste("NORCLOZ_WHOLE.genome", cols, sep = "_"))
...
COGS_CLOZ = all_prs_CLOZ,
COGS_NORCLOZ = all_prs_NORCLOZ,
COGS_RATIO = all_prs_RATIO,
...
rows_to_keep_SCZ_broad_minus_narrow <- COGS_total[DSM_diagnosis_new == 3 | DSM_diagnosis_new == 4]
...
rows_to_keep_SCZ_broad_minus_narrow_cognition <- rows_to_keep_SCZ_broad_minus_narrow[Composite_Zimpute != 999]
rows_to_keep_SCZ_broad_minus_narrow_TRS <- rows_to_keep_SCZ_broad_minus_narrow[FINAL_TRS != 9]
...
list_of_tests_cognition <- list(
  ...
  rows_to_keep_SCZ_broad_minus_narrow_cognition = as.data.frame(rows_to_keep_SCZ_broad_minus_narrow_cognition[, test := Composite_Zimpute]),
...
list_of_tests_TRS <- list(
  ...
  rows_to_keep_SCZ_broad_minus_narrow_TRS = as.data.frame(rows_to_keep_SCZ_broad_minus_narrow_TRS[, test := FINAL_TRS]),
...
all_cogs_PRS_results <- list(
  ...
  Cognition_broad_minus_narrow = results_rows_to_keep_SCZ_broad_minus_narrow_cognition
  )
...
all_TRS_PRS_results <- list (
  ...
  TRS_broad_minus_narrow <- results_rows_to_keep_SCZ_broad_minus_narrow_TRS
)

library(knitr)

samples <- c("Everything", "broad", "narrow", "Broad_minus_narrow")

for (i in 1:length(samples)){
  all_TRS_PRS_results[[i]] <- cbind(all_TRS_PRS_results[[i]], samples[i])
} 

Full_data <- ldply(all_TRS_PRS_results, data.frame)
Full_data <- as.data.table(Full_data)

keep_columns <- c("FID", "IID", PRS_names,"narrow_DSM" ,"broad_DSM", "broad_minus_narrow_DSM")


Testing_COGS_TRS <- COGS_total

rows_to_keep_SCZ_narrow_integer <- na.omit(COGS_total[,.I[DSM_diagnosis_new == 1 | DSM_diagnosis_new == 2]])
rows_to_keep_SCZ_broad_integer <- na.omit(COGS_total[,.I[DSM_diagnosis_new == 1 | DSM_diagnosis_new == 2 | DSM_diagnosis_new == 3 | DSM_diagnosis_new == 4]])
rows_to_keep_broad_minus_narrow_integer <- na.omit(COGS_total[,.I[DSM_diagnosis_new == 3 | DSM_diagnosis_new == 4]])
rows_to_keep_is_na <- COGS_total[,.I[is.na(DSM_diagnosis_new)]]

Testing_COGS_TRS[, narrow_DSM := 0]
Testing_COGS_TRS[rows_to_keep_SCZ_narrow_integer,narrow_DSM := 1]
Testing_COGS_TRS[rows_to_keep_is_na,narrow_DSM := NA]

Testing_COGS_TRS[, broad_DSM := 0]
Testing_COGS_TRS[rows_to_keep_SCZ_broad_integer, broad_DSM := 1]
Testing_COGS_TRS[rows_to_keep_is_na, broad_DSM := NA]

Testing_COGS_TRS[, broad_minus_narrow_DSM := 0]
Testing_COGS_TRS[rows_to_keep_broad_minus_narrow_integer, broad_minus_narrow_DSM := 1]
Testing_COGS_TRS[rows_to_keep_is_na, broad_minus_narrow_DSM := NA]

Testing_COGS_TRS_minus_na <- Testing_COGS_TRS[-rows_to_keep_is_na]

Testing_COGS_TRS_minus_na <- Testing_COGS_TRS_minus_na[, c(keep_columns), with = F]
...
write.table(Testing_COGS_TRS_minus_na,file = paste0(Location_of_analysis,Name_of_analysis,"_PRS_Profiles_for_shiny",time_of_analysis,".txt"), row.names = F, quote = F)

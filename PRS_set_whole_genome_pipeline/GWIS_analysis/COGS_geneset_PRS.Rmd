...
Cognition_phenotype_names <- c("Composite_Zimpute","ZHVLTTotal", "ZNABMazes", "ZBVMTTtotal", "ZMSCEIT", "ZCPTMean", "SoP_Zimpute", "WM_Zimpute")
Cognition_phenotype_list <- list()

for (k in 1:length(Cognition_phenotype_names)){
...
list_of_tests_cognition <- list(
  rows_to_keep_SCZ_narrow_cognition = as.data.frame(rows_to_keep_SCZ_narrow_cognition[, test:= (rows_to_keep_SCZ_narrow_cognition[,Cognition_phenotype_names[k], with = F])]),
  rows_to_keep_SCZ_broad_cognition = as.data.frame(rows_to_keep_SCZ_broad_cognition[, test:= (rows_to_keep_SCZ_broad_cognition[,Cognition_phenotype_names[k], with = F])]),
  rows_to_keep_SCZ_all_minus_narrow_cognition = as.data.frame(rows_to_keep_all_minus_narrow_cognition[, test:= (rows_to_keep_all_minus_narrow_cognition[,Cognition_phenotype_names[k], with = F])]),
  COGS_cognition = as.data.frame(COGS_cognition[,test:= (COGS_cognition[,Cognition_phenotype_names[k], with = F])])
  )  
...
Cognition_phenotype_list[[k]] <- all_cogs_PRS_results <- list(

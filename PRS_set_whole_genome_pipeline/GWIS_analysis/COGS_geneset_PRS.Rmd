...
obj <- data.frame(test=0, score=0, estimate=0, SE=0, tvalue=0, p=0, r.squared=0, lower = 0, upper = 0)
  ...
  fit <- lm (Composite_Zimpute ~ narrow_DSM, data = Testing_COGS_cognition)
  fit2 <- lm (Composite_Zimpute ~ Broad_DSM, data = Testing_COGS_cognition)
  fit3 <- lm (Composite_Zimpute ~ All_minus_narrow_DSM, data = Testing_COGS_cognition)
...
results_narrow_DSM_comparison <- results

# Whole_genome_DSM_check
obj <- data.frame(test=0, score=0, estimate=0, SE=0, tvalue=0, p=0, r.squared=0, lower = 0, upper = 0)

for (i in score_list) {
  fit <- lm (test ~ COGS_cognition$i + male_sex + Age_at_Interview + Broad_DSM, data = COGS_cognition)
  fit2 <- lm (test ~ male_sex + Age_at_Interview + Broad_DSM, data = COGS_cognition)
  tmp <- coef(summary(fit))
  tmp2 <- summary(fit)
  hold <- summary(fit2)
  true_r2 <- tmp2$r.squared - hold$r.squared
...
write.table(Full_data,file = paste0(Location_of_analysis,Name_of_analysis,".txt"), row.names = F, quote = F)

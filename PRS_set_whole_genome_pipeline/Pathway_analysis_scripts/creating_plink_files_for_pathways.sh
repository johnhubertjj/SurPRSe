...
length_of_array=`echo "$((${length_of_array} - ${num1}))"`	
	...
	echo ${number_of_files[i]}
        plink --bfile CLOZUK_GWAS_BGE_chr${number_of_files[i]}_magma_input --extract chromosome_${number_of_files[i]}_${pathways[a]}SNPs_for_clumping.txt --make-bed --out chromosome_${number_of_files[i]}_CLOZUK_PGC_${pathways[a]}		 
	done
done

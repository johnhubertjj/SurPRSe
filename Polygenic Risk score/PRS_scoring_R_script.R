...
setnames(combined.CLOZUK.PGC.clumped.Genomic.SNPs, "GENE", "Gene_ID")
...
for (i in 1:length(p.value.thresholds)) {
  ...
  SNPs <- a[, .I[which(Gene_ID == Genes_index[l] & P <= p.value.thresholds[i])]]
  
    if (length(SNPs) != 0){
    a <- a[SNPs, .(SNP,A1,BETA)]
  
    filename <- paste0('./CLOZUK_PGC_PRS_test/score/chr22_test_', Genes_index[l],'_', p.value.thresholds[i],".score")
  

    write.table(file = filename, a, row.names = F, col.names = F, quote = F, sep="\t")
  
    command <- paste0('plink --bfile ./output/CLOZUK_GWAS_BGE_CLUMPED_chr22 --score ', filename, " --out /Users/JJ/Dropbox/testing_PRS_chromosome_22/CLOZUK_PGC_PRS_test/Profiles/chr22_test_", Genes_index[l], "_", p.value.thresholds[i], "_a")
    system(command)
    rm(a)
